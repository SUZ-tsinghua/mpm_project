...
def process_frame(frame_dir, rank, args):
    os.system(
        command.format(
            args.scene_file, args.device_type, rank % num_gpus, frame_dir,
            os.path.join(frame_dir, args.rendered_image_name), "" if
            (rank == 0 and not args.quiet) else " > /dev/null 2>&1")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rendered_image_name',
                        type=str,
                        default='render.png')
